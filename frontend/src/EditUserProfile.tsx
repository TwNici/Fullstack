...
function EditUserProfile() {
    ...
    useEffect(() => {
        axios.get("/api/user/mitarbeiter/specific", {
            ...
            setFormData({
                ...
                geschaeftsadresse: res.data.geschaeftsadresse,
                rolle: res.data.rolle,
                ort: res.data.ort,
                gebaeude: res.data.gebaeude,
                bildUrl: res.data.bildUrl,
                stock: res.data.stock,
                userid: res.data.userid,
                telefonnummer: res.data.telefonnummer,
                pultnummer: res.data.pultnummer
            });
        });
    }, []);
    ...
    const handlePwSubmit = () => {
        if (passwordData === passwordData2){
            axios.post("/api/user/mitarbeiter/password", { password: passwordData }, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem("jwt")}`
                }
            }).then(() => {
        ...
        <div>
            ...
            <div id="editdatencanvasPW" className="canvas shadow-and-radius">
                <div className="InputfelderDesign">
                    <form onSubmit={(e) => {e.preventDefault(); handlePwSubmit(); }}>
                        ...
                        <input id="InputFelderEdit2" placeholder="Neues Passwort Wiederholen" type="Password" onChange={(e) => handlePwChange2(e.target.value)} />
            ...
            <div id="editdatencanvasPICTURE" className="canvas shadow-and-radius">
                <form onSubmit={(e) => {e.preventDefault(); handleProfileSubmit(); }}>
                    ...
                    <input ref={fileInputRef} id="dragDropEdit" type="file" name="bildUrl" onChange={handleFileChange} />
                    <button className={"btn-layout shadow-and-radius"} id={"editSaveButtonIMG"} type={"submit"}>Save</button>
                </form>
            </div>

            <img src={formatBase64Image(formData.bildUrl)} id="showeditPicture" className="canvas shadow-and-radius" alt="" />
